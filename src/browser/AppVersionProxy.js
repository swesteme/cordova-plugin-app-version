...
//added by @Kevin Iuretig
getAppVersion.getMetaData = function (property, success, fail) {
    return readCustomConfig(property, function (data) {
        success(data);
    }, fail);
};

function readCustomConfig(property, success, fail){
    var xhr = new XMLHttpRequest();
    xhr.addEventListener("load", function () {
        try {
            var parser = new DOMParser();
            ...
            var data = widget.querySelectorAll('platform[name=browser] meta-data').getAttribute(property);
